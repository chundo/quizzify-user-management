...
export default class EditableSelect extends React.Component {
  ...
  save = (event) => {
    ...
    this.props.onUpdate(this.refs.el.name, this.refs.el.value, this.props.placeholder);
    ...
    if (this.state.isEditing) {
      ...
      return (
        <XEditable isLoading={false} save={this.save} cancel={this.cancel}>
          <select ref='el' className={selectClassName} id={this.props.id} name={this.props.name} defaultValue={this.state.value} placeholder={this.props.placeholder}>
            {options}
          </select>
        </XEditable>
      );
